...
def check_radar_availability(solution, radar1_height, radar2_height):
    radar1_is_available, radar2_is_available = True, True
    ...
    return radar1_is_available, radar2_is_available
...
def extract_coordinates_from_chromosome(solution, grid):
    ...
    return x1, y1, x2, y2, radar1_height, radar2_height
...
def fitness_function(solution, solution_idx):
    ...
    grid = pd.read_csv('./data/grids/Grid_50x50.csv', header=None)
    radars = pd.read_csv('./data/radars_small.csv')

    x1, y1, x2, y2, radar1_height, radar2_height = extract_coordinates_from_chromosome(solution, grid)

    radar1_is_available, radar2_is_available = check_radar_availability(solution, radar1_height, radar2_height)
...
def mutation_func(offspring, ga_instance):
    ...
    for chromosome_idx in range(offspring.shape[0]):
        ...
        mutated_offspring[chromosome_idx, random_gene_idx] = offspring[
                                                                 chromosome_idx, random_gene_idx] + np.random.randint(
            low, high)
...
def make_visualization(best_solution):
    ...
    x1, y1, x2, y2, radar1_height, radar2_height = extract_coordinates_from_chromosome(best_solution, grid)

    radar1_is_available, radar2_is_available = check_radar_availability(best_solution, radar1_height, radar2_height)
...
if __name__ == "__main__":
    ...
    with open('radars_position.csv', 'w', newline='') as file:
